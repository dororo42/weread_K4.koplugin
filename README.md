...
- **书内注释（脚注）显示**：新增「脚注显示位置」设置（设置 → 下载设置）：
  - **显示在每章末尾**（默认）：注释集中在章节末尾，单份显示（不依赖引擎脚注机制，K4 上保证不重复）；[N] 标记跳到章末对应注释
  - **显示在引用它的当页底部**（实验性）：CREngine 页内脚注渲染，长注释可延伸到次页；K4 部分固件上可能同时在章节末尾保留副本（引擎级行为，插件已折叠原位置副本以消除重复，若仍重复请用默认模式）
  - 注释内返回链接均可跳回正文引用处；切换设置后需重新下载已缓存的书生效

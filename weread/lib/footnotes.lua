...
-- Ported from upstream PR #133 (2026-09-05): decode the UTF-8 codepoint
-- starting at byte i; returns the codepoint (nil for a malformed sequence)
...
-- Ported from upstream PR #133 (2026-09-05). Backlink glyphs (↩ ↑ ← ⤴) are
-- pure symbols, yet an element carrying one may share the note target's id;
-- such a capture can never be a definition. Enumerating "known" text scripts
-- instead (the old %w + CJK lead-byte check) silently dropped notes written
-- only in kana, hangul, Cyrillic or any other non-ASCII script, so reject a
-- candidate only when EVERY codepoint it holds is a symbol or punctuation:
-- arrows, dingbats and enclosed numbers (U+2000-U+2FFF), CJK punctuation
-- (U+3000-U+303F), Latin-1 symbols (U+0080-U+00BF), variation selectors and
-- the BOM. Any other codepoint (Latin letters, kana, hangul, CJK, emoji …)
-- counts as note text.
...
-- Ported from upstream PR #133 (2026-09-05): cap on cleaned note text length
-- in bytes (#text counts bytes, so this is ~2000 CJK chars); genuine notes
-- are far smaller, whole-chapter captures are far larger.
...
local function remember_definition(definitions, anchor, inner)
    ...
    -- Ported from upstream PR #133: a capture consisting entirely of symbol
    -- codepoints (backlink arrows sharing the note id) can never be a
    -- definition — reject before it can occupy the anchor slot.
    if is_symbol_only(text) then return end
    -- Ported from upstream PR #133: ancestor blocks spanning most of the
    -- chapter poison the definition; reject oversized candidates outright
    ...
    -- Ported from upstream PR #133: the true note content is the smallest
    -- region describing the anchor, so keep whichever candidate has the
    -- shortest cleaned text (replaces the old first-wins behaviour).

...
function ReadStatsView:init()
    ...
    if Device:hasKeys() then
        self.key_events = {
            ...
            PrevPeriod = { { "Left" } },
            NextPeriod = { { "Right" } },
            -- K4 non-touch has no number row (no physical keyboard at all):
            -- the D-pad center key cycles week/month/year/total, the D-pad
            -- up/down and both page-turn keys scroll the page. Left/Right stay
            -- on period navigation. The number-row bindings below only fire on
            -- devices with a physical keyboard (Kindle Keyboard: Alt+Q/W/E/R).
            SwitchTabNext = { { "Press" } },
            SwitchTabWeek = { { "1" } },
            SwitchTabMonth = { { "2" } },
            SwitchTabYear = { { "3" } },
            SwitchTabOverall = { { "4" } },
            ScrollUp = { { "Up" }, { "PageUp" }, { "LPgBack" }, { "RPgBack" } },
            ScrollDown = { { "Down" }, { "PageDown" }, { "LPgFwd" }, { "RPgFwd" } },
        }
    end
...
-- K4 non-touch key handlers: tab switching and scrolling.
function ReadStatsView:onSwitchTabNext()
    for i, tab in ipairs(TABS) do
        if tab.mode == self.data.mode then
            local next_tab = TABS[(i % #TABS) + 1]
            return self:onSwitchMode(next_tab.mode)
        end
    end
    return true
end

function ReadStatsView:onSwitchTabWeek()
    return self:onSwitchMode("weekly")
end

function ReadStatsView:onSwitchTabMonth()
    return self:onSwitchMode("monthly")
end

function ReadStatsView:onSwitchTabYear()
    return self:onSwitchMode("annually")
end

function ReadStatsView:onSwitchTabOverall()
    return self:onSwitchMode("overall")
end

function ReadStatsView:onScrollUp()
    if self.scroll then
        return self.scroll:onScrollPageUp() or true
    end
    return true
end

function ReadStatsView:onScrollDown()
    if self.scroll then
        return self.scroll:onScrollPageDown() or true
    end
    return true
end
